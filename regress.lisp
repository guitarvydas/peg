(DEFPACKAGE :RPEG-GRAMMAR (:USE :CL :ESRAP :CL-HEREDOC))
(IN-PACKAGE :RPEG-GRAMMAR)
(DEFRULE GRAMMAR (AND SPACING (+ DEFINITION) SPACING ENDOFFILE)
  (:DESTRUCTURE
   (SPC DEF SPC2 EOF)
   (DECLARE (IGNORE SPC EOF SPC2))
   `(PROGN (DEFPACKAGE :RPEG-GRAMMAR (:USE :CL :ESRAP :CL-HEREDOC)) (IN-PACKAGE :RPEG-GRAMMAR) ,@DEF)))
...
(DEFRULE EXPRESSION (AND SEQUENCE (* SLASHSEQUENCE)) (:DESTRUCTURE (SEQ SEQS) (IF SEQS `(OR ,SEQ ,@SEQS) SEQ)))
(DEFRULE SLASHSEQUENCE (AND SLASH SEQUENCE) (:DESTRUCTURE (SL SEQ) (DECLARE (IGNORE SL)) SEQ))
(DEFRULE SEQUENCE (* PREFIX)
  (:DESTRUCTURE (&REST PREF) (IF PREF (IF (AND (CONSP PREF) (> (LENGTH PREF) 1)) `(AND ,@PREF) (FIRST PREF)) (VALUES))))
(DEFRULE PREFIX (AND (? (OR AND NOT)) SUFFIX) (:DESTRUCTURE (PREF SUFF) (IF PREF (LIST PREF SUFF) SUFF)))
(DEFRULE SUFFIX (AND PRIMARY (? (OR QUESTION STAR PLUS))) (:DESTRUCTURE (PRIM SUFF) (IF SUFF (LIST SUFF PRIM) PRIM)))
(DEFRULE PRIMARY (OR P1 P2 LITERAL RCLASS DOT) (:LAMBDA (X) X))
...
(DEFRULE NOTSINGLE (AND (! (CHARACTER-RANGES #\')) CHAR) (:FUNCTION SECOND))
(DEFRULE NOTDOUBLE (AND (! (CHARACTER-RANGES #\")) CHAR) (:FUNCTION SECOND))
(DEFRULE RCLASS (AND "[" (* NOTRB) "]" SPACING)
  (:DESTRUCTURE
   (LB RANGE RB SPC)
...
(DEFRULE CHARRANGE (AND CHAR "-" CHAR) (:DESTRUCTURE (C1 DASH C2) (DECLARE (IGNORE DASH)) (LIST C1 C2)))
(DEFRULE SINGLECHAR CHAR (:LAMBDA (C) C))
...
(DEFRULE AND (AND "&" SPACING) (:LAMBDA (LIST) (DECLARE (IGNORE LIST)) 'AND))
(DEFRULE NOT (AND "!" SPACING) (:LAMBDA (LIST) (DECLARE (IGNORE LIST)) '!))
...
(DEFRULE SPACING (* (OR RSPACE COMMENT)) (:LAMBDA (LIST) (DECLARE (IGNORE LIST)) (VALUES)))
...
(DEFRULE RSPACE (OR " " "	" ENDOFLINE) (:LAMBDA (LIST) (DECLARE (IGNORE LIST)) (VALUES)))
(DEFRULE ENDOFLINE
    (OR "
"
        "
